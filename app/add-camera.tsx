...
import { ArrowLeft, Camera, Wifi, WifiOff, Check, AlertCircle } from 'lucide-react-native';
...
export default function AddCameraScreen() {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CameraForm, string>>>({});

  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CameraForm, string>> = {};
  ...
  const updateField = (field: keyof CameraForm, value: string) => {
        ...
        <View className="px-6 py-6 gap-5">
            ...
            <View className="flex-row gap-3">
              <TouchableOpacity
                ...
                className={`flex-1 border-2 ${
                  form.protocol === 'RTSP' ? 'border-primary bg-primary/10' : 'border-border bg-card'
              ...
              <TouchableOpacity
                ...
                className={`flex-1 border-2 ${
                  form.protocol === 'WebRTC' ? 'border-primary bg-primary/10' : 'border-border bg-card'
                } rounded-xl py-3 items-center`}
              >
          ...
          <TouchableOpacity
            ...
            className={`border-2 border-dashed ${
              testResult === 'success' ? 'border-green-500 bg-green-500/10' :
